...
export class Simulator extends plugin {
    ...
    async hundredPull(e) {
        ...
        let forwardMsg
        if (e.isGroup) {
            forwardMsg = await common.makeForwardMsg(e, imgs, '点我查看抽卡结果')
        } else {
            forwardMsg = imgs
        }
        ...
        await e.reply([
            `〓〓百连抽卡结果〓〓\n`,
            `★★★★★ 获得 ${fiveStarItems.length} 个\n`,
            ...fiveStarItems.map(item => `· ${item.name}`),
            `\n★★★★ 获得 ${fourStarItems.length} 个\n`,
            ...fourStarItems.map(item => `· ${item.name}`),
            `\n点击上方查看详细抽卡记录`
        ].join('\n'))
